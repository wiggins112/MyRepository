...
<template>
  <div class="root">
    <div v-for="(item, index) in cItems" :key="item.key" :accessible="true" :aria-label="`${item.title},${item.desc?item.desc:''},${item.date?item.date:''},${item.highlight?$t('已完成'):$t('等待完成')}`" @click="clickSimpleFlowItem(item, index)">
      ...
      <div class="content full-rest" :class="item.__contentClass__">
        <div class="line" :class="item.__contentLineClass__" :style="item.__lineStyle__"></div>
        <div class="full-rest">
            <text v-if="listCn" class="trans_title">{{listCn[index]}}</text>
...
export default {
  ...
  methods: {
    clickSimpleFlowItem(item, index) {
      this.$emit('clickSimpleFlowItem', {
        item: item,
        index: index
      })
    },
