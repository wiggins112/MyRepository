...
<template>
  <div class="wrapper">
    ...
    <div ref="wxc-popover"
         ...
         v-if="show && buttons"
         :style="contentStyle">
      <div class="u-popover-arrow" :style="arrowStyle" v-if="hasArrow"></div>
      <slot>
        <div class="u-popover-inner">
            <div :class="['i-btn',i==buttons.length-1?'i-btn-noborder':'']"
              ...
              <text :class="['btn-text', activeIndex === i ? 'btn-text-active' : '']">{{item.text}}</text>
            </div>
        </div>
      </slot>
    </div>
